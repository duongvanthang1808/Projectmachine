...
{
    public partial class frmBCmaykhongbanduoc : Form
    {
        DataTable tblBanHang;
        public frmBCmaykhongbanduoc()
        {
            InitializeComponent();
        }
        string str;
        private void frmBCmaykhongbanduoc_Load(object sender, EventArgs e)
        {
            int i;
            for (i = 1; i <= 4; i++)
                cboQuy.Items.Add(i);
        ...
        {
            if (cboThang.Text != "") btnBaocao.Enabled = true;
        }
        ...
        {
            string sql = "SELECT a.Mamay, a.Tenmay, b.Tenloai, a.Soluong, a.Giaban  FROM tblKhomay AS a, tblLoaimay AS b WHERE a.Maloai = b.Maloai";
            tblBanHang = Functions.GetDataToTable(sql);
            dvgBanHang.DataSource = tblBanHang;
            dvgBanHang.Columns[0].HeaderText = "Mã máy";
            dvgBanHang.Columns[1].HeaderText = "Tên sách";
            dvgBanHang.Columns[2].HeaderText = "Tên loại sách";
            dvgBanHang.Columns[3].HeaderText = "Số lượng";
            dvgBanHang.Columns[4].HeaderText = "Đơn giá bán";
            ...
            {
                str = "SELECT c.Mamay FROM tblChitietHDB AS c, tblHoadonban AS b WHERE (MONTH(b.Ngayban)-1)/3 + 1 = " + cboQuy.Text + " AND YEAR(b.Ngayban) = " 
                    + cboNam.Text+ " AND MONTH(b.Ngayban)= " + cboThang.Text;
                // str= "SELECT a.Masach, a.Tensach, b.Tenloai, a.Soluong, a.Dongiaban FROM tblKhosach AS a, tblLoaisach AS b WHERE a.Maloaisach = b.Maloaisach AND a.Masach not in str";
                btnIn.Enabled = true;
            }
            else
            {
        ...
        private void btnIn_Click(object sender, EventArgs e)
        {
            ...
            exRange.Range["A1:B1"].Value = "Cửa hàng máy tính Trần Anh";
            ...
            exRange.Range["B8:B8"].Value = "Mã máy";
            exRange.Range["C8:C8"].Value = "Tên sách";
            exRange.Range["D8:D8"].Value = "Tên loại sách";
            exRange.Range["E8:E8"].Value = "Số lượng";
            exRange.Range["F8:F8"].Value = "Đơn giá bán";
